...
<x-container :title="$title">
   <x-header/>
    ...
    <main>
        ...
        <section x-data="product" class="tp-product-details-area">
                    ...
                    <div class="col-xl-7 col-lg-6">
                       <div class="tp-product-details-thumb-wrapper tp-tab d-sm-flex">
                          <nav>
                             <div class="nav nav-tabs flex-sm-column " id="productDetailsNavThumb" role="tablist">
                                @foreach ($product['productImage'] as $key => $image)
                                    <button class="nav-link {{$key == 0 ? 'active' : ''}}" id="nav-{{$image['id']}}-tab" data-bs-toggle="tab" data-bs-target="#nav-{{$image['id']}}" type="button" role="tab" aria-controls="nav-{{$image['id']}} aria-selected="false"">
                          ...
                          <div class="tab-content m-img" id="productDetailsNavContent">
                            @foreach ($product['productImage'] as $key => $image)
                                <div class="tab-pane fade {{$key == 0 ? 'show active' : ''}}" id="nav-{{$image['id']}}" role="tabpanel" aria-labelledby="nav-{{$image['id']}}-tab" tabindex="0">
                       ...
                       <div class="tp-product-details-wrapper">
                          ...
                          <div class="tp-product-details-inventory d-flex align-items-center mb-10">
                             <div class="tp-product-details-stock mb-10">
                                <span x-text="quantity ? `Số lượng: ${quantity}` : 'Còn hàng'"></span>
                             </div>
                          ...
                          <div class="tp-product-details-price-wrapper mb-20">
                             ...
                             <span x-text="printPrice($data)"class="tp-product-details-price new-price"></span>
                          </div>
  
                          <!-- variations -->
                          <div class="product-variation mb-30">
                             <!-- single item -->
                             <template x-for="variation,index in product.variations" :key="variation.id">
                              <div class="mb-15">
                                 <h4 class="variation-label" x-text="variation.name"></h4>
                                 <div class="variation-btn-wrap">
                                    <template x-for="option in variation.option" :key="option.id">
                                       <button 
                                       @mouseenter="handleMouseEnterVariation($event,$data,option,index)"
                                       @mouseleave="handleMouseLeaveVariation($event)"
                                       @click="activeButtonVariation($event,$data,option,index)" type="button" class="variation-btn" x-text="option.value"></button>
                                    </template>
                                 </div>
                              </div>
                             </template>
                             ...
                             <div class="tp-product-details-action-item-wrapper d-flex align-items-center">
                                <div class="tp-product-details-quantity">
                                   <div class="tp-product-quantity mb-15 mr-15">
                                      <span class="tp-cart-minus" style="user-select:none" @click="changeProductQuantity($data,2)">
                                         <svg width="11" height="2" viewBox="0 0 11 2" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M1 1H10" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                         </svg>                                                            
                                      </span>
                                      <input @keypress="onlyEnterNumber($event,$data)" 
                                      @focusout="checkInputValid($data)"
                                      class="tp-cart-input" type="text" x-model.lazy="productItem.quantity">
                                      <span class="tp-cart-plus" style="user-select:none" @click="changeProductQuantity($data,1)">
                                ...
                                <div class="tp-product-details-add-to-cart mb-15 w-100">
                                   <button @click="addToCart($store,$data)" class="tp-product-details-add-to-cart-btn w-100">Thêm vào giỏ</button>
                                </div>
                             </div>
                             <button class="tp-product-details-buy-now-btn w-100" @click="buyNow($data)">Mua ngay</button>
                          </div>
                          <div class="tp-product-details-msg mb-15">
                            <p>Chỗ in thông tin shop</p>
                          </div>
                       </div>
                    </div>
                 </div>
                       ...
                       <div class="tp-product-details-tab-nav tp-tab">
                          <nav>
                             <div class="nav nav-tabs justify-content-center p-relative tp-product-tab" id="navPresentationTab" role="tablist">
                               <button class="nav-link" id="nav-description-tab" data-bs-toggle="tab" data-bs-target="#nav-description" type="button" role="tab" aria-controls="nav-description" aria-selected="true">Mô tả sản phẩm</button>
                               <button class="nav-link active" id="nav-addInfo-tab" data-bs-toggle="tab" data-bs-target="#nav-addInfo" type="button" role="tab" aria-controls="nav-addInfo" aria-selected="false">Thông tin sản phẩm</button>
     ...
     @push('scripts')
        <script>
            const product = {{Js::from($product)}}

            document.addEventListener('alpine:init',() => {
               Alpine.data('product',() => ({
                  product,
                  selectedVariations: [],
                  productItem: {id: undefined,quantity: 1},
                  ...
                  init(){
                     ...
                     this.product.variations.forEach(item => {
                        that.selectedVariations.push('')
                     })

                     this.product.product_item.forEach(item => {
                        that.maxPrice = Math.max(that.maxPrice,item.price)
                        that.minPrice = Math.min(that.minPrice,item.price)
                     })

                     if (this.product.product_item.length == 1){
                        const productItem =  mapVariation.get('')
                        this.productItem.id = productItem.id
            ...
            let prevVariElemActive = [];
            product.variations.forEach(item => {
               prevVariElemActive.push(null)
            })
            ...
            function printPrice(data) {
                 ...
                 if (!data.price ){
                     if (data.minPrice == data.maxPrice){
                        return formater.format(data.minPrice)
                     }
                     return `${formater.format(data.minPrice)} - ${formater.format(data.maxPrice)}`
                 }
                 else{
                     return formater.format(data.price)
                 }
            }
            
            function activeButtonVariation(event,data,option,index) {
               if (prevVariElemActive[index]){
                  prevVariElemActive[index].classList.remove('active')
               }
               event.target.classList.add('active')
               prevVariElemActive[index] = event.target

               data.selectedVariations[index] = option.id
               
               const isFull = !data.selectedVariations.some(item => !item)
               if (isFull){
                  data.selectedVariations.sort((a,b) => a-b)
                  const key = data.selectedVariations.join('-')
                  const productItem = mapVariation.get(key)
                  data.price = productItem.price
                  data.quantity = productItem.quantity

                  data.productItem.id = productItem.id
                  console.log(mapVariation.get(key));
               }
            }

            function handleMouseEnterVariation(event,data,option,index) {
               const fakeSelected = data.selectedVariations.slice()
               fakeSelected[index] = option.id
               const isFull = fakeSelected.every(item => item)
               if (isFull){
                  fakeSelected.sort((a,b) => a-b)
                  const key = fakeSelected.join('-')
                  const productItem = mapVariation.get(key)
            ...
            function handleMouseLeaveVariation(event) {
            ...
            function onlyEnterNumber(event,data){
               if (!data.productItem.id){
                  toastr.error("Bạn chưa chọn phân loại sản phẩm")
            ...
            function checkInputValid(data) {
               if (!data.productItem.quantity){
                  data.productItem.quantity = 1
               }
               else if (data.productItem.quantity > data.quantity){
                  data.productItem.quantity = data.quantity
               }
            }

            function changeProductQuantity(data,mode) {
               if (!data.productItem.id){
                  toastr.error("Bạn chưa chọn phân loại sản phẩm")
                  return
               }
               if (mode == 1 && data.productItem.quantity < data.quantity){
                   data.productItem.quantity++
               }
               else if (mode == 2 && data.productItem.quantity > 1){
                  data.productItem.quantity--
               }
            }

            function addToCart(store,data) {
               if (data.productItem.id){
                  $.ajax({
                     url: "api/cart",
                     method: 'POST',
                     ...
                     success: function (res) {
                        if (res.message == 'success'){
                          toastr.success('Đã thêm sản phẩm vào giỏ hàng')
                           if (store.cart.items.every(item => item.product_item.id != data.productItem.id)){
                     ...
                     success: function (res) {
                        if (res.message == 'success'){
                           ...
                           res.data.forEach(item => {
                              if (item.checked && listChecked && !listChecked.includes(item.id)){
            ...
            function initMapVariation(){
               product.product_item.forEach(item => {
                  const options = []
                  item.variation_option.forEach(item1 => {
                     options.push(item1.id)
                  })
                  options.sort((a,b) => a - b)
                  const key = options.join('-')
                  mapVariation.set(key,item)
               })
            }
            initMapVariation()
            console.log(mapVariation);
